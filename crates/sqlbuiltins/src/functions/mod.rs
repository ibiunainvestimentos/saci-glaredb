...
impl FunctionRegistry {
    pub fn new() -> Self {
        ...
        let udfs: Vec<Arc<dyn BuiltinScalarUDF>> = vec![
            ...
            Arc::new(HasColumnPrivilege),
            Arc::new(HasTypePrivilege),
            Arc::new(HasFunctionPrivilege),
            Arc::new(PgHasRole),
            ...
            Arc::new(PgFunctionIsVisible),
            Arc::new(PgTypeIsVisible),
            Arc::new(PgOpclassIsVisible),
            Arc::new(PgConversionIsVisible),
            Arc::new(PgCollationIsVisible),
            Arc::new(PgEncodingToChar),
            Arc::new(PgArrayToString),
            Arc::new(PgVersion),
            Arc::new(FormatType),
            // pg_get_* family — DDL synthesis stubs (real bodies follow).
            Arc::new(PgGetExpr),
            Arc::new(PgGetViewdef),
            Arc::new(PgGetIndexdef),
            Arc::new(PgGetConstraintdef),
            Arc::new(PgGetTriggerdef),
            Arc::new(PgGetFunctiondef),
            Arc::new(PgGetFunctionArguments),
            Arc::new(PgGetFunctionIdentityArguments),
            Arc::new(PgGetFunctionResult),
            Arc::new(PgGetPartkeydef),
            Arc::new(PgTablespaceLocation),
            // OID-resolution (`to_regclass` etc. — psycopg3 typeinfo).
            Arc::new(ToRegtype),
            Arc::new(ToRegclass),
            Arc::new(ToRegproc),
            // Misc PG helpers
            Arc::new(PgMyTempSchema),
            // Sizing — Delta tables not measured; all return 0.
            Arc::new(PgRelationSize),
            Arc::new(PgTotalRelationSize),
            Arc::new(PgIndexesSize),
            Arc::new(PgDatabaseSize),
            Arc::new(PgTableSize),
            Arc::new(PgSizePretty),
            // Comments — NULL until COMMENT ON storage lands.
            Arc::new(ObjDescription),
            Arc::new(ColDescription),
            Arc::new(ShobjDescription),
            // Arrow-name → PG OID resolver, used by pg_attribute /
            // pg_proc to fill atttypid / prorettype dynamically.
            Arc::new(PgTypeOidByName),
            // Connection / process metadata.
            Arc::new(PgBackendPid),
            Arc::new(PgIsInRecovery),
            Arc::new(PgPostmasterStartTime),
            // System functions
